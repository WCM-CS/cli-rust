...
// Unkown section -- Either we use conditions to determine which protein sources to load or choopse at random then potentially have to select the other optioon whihc would require additional calls to the proxy,
...
// NOTE: potential error if we chose wrong food options since going back at this pout would require loading all values again aka starting over
